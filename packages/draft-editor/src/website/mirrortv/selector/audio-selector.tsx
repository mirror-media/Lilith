    ...
    <AudioGridsWrapper>
      {audios.map((audio) => {
        return (
          <AudioGrid
            key={audio.id}
            isSelected={selected?.some((s) => s.id === audio.id)}
...
function AudioGrid(props: {
  ...
  return (
    <AudioGridWrapper key={audio?.id} onClick={() => onSelect(audio)}>
      ...
      <AudioName>{audio?.name || 'Untitled'}</AudioName>
    </AudioGridWrapper>
  )
}
...
const AudiosQuery = gql`
  query Audios($searchText: String!, $take: Int, $skip: Int) {
    audiosCount(where: { name: { contains: $searchText } })
    audios(
...
export function AudioSelector(props: { onChange: AudioSelectorOnChangeFn }) {
  const [
    queryAudios,
    { loading, error, data: { audios = [], audiosCount = 0 } = {} },

# @mirrormedia/lilith-openrelationship
...
關於 lilith-openrelationship 中使用到哪些環境變數，可以至 [`environment-variables.ts`](https://github.com/mirror-media/Lilith/blob/main/packages/openrelationship/environment-variables.ts) 查看。

## Getting started on local environment
### Start postgres instance
在起 lilith-openrelationship 服務前，需要在 local 端先起 postgres database。
...
```
docker run -p 5433:5432 --name lilith-openrelationship -e POSTGRES_PASSWORD=passwd -e POSTGRES_USER=account -e POSTGRES_DB=lilith-openrelationship -d postgres
```

註：
`POSTGRES_PASSWORD`, `POSTGRES_USER` 和 `POSTGRES_DB` 都可更動。
只是要注意，改了後，在起 lilith-openrelationship 的服務時，也要更改傳入的 `DATABASE_URL` 環境變數。
...
### Start dev instance
確定 postgres 服務起來和相關套件安裝完畢後，可以執行以下 command 來起 lilith-openrelationship 服務
```
yarn dev
// or
npm run dev
...
### GraphQL playground
起 lilith-openrelationship CMS 服務後，我們可以透過 [http://localhost:3000/api/graphql](http://localhost:3000/api/graphql) 來使用 GraphQL playground。

### Start GraphQL API server only
我們也可以單獨把 lilith-openrelationship 當作 GraphQL API server 使用。

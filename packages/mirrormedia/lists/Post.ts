      ...
      access: {
        read: ({ context, item }) => {
          if (envVar.accessControlStrategy === 'gql') {
            // `context.req.decodedAccessToken` is generated by
            // a express middleware.
            // This middleware will check the request authorization
            // by verifying the request's JWT access token and decode
            // the JWT. The decoded payload is stored in

import { humanize } from '../utils/text/utils'
import {
  BaseListTypeInfo,
  fieldType,
  FieldTypeFunc,
...
} from '../utils/non-null-graphql'
import { CustomTimestampFieldMeta } from './views'
import { resolveView } from '../resolve-view'

export type CustomTimestampFieldConfig<ListTypeInfo extends BaseListTypeInfo> =
...
export const CustomTimestamp =
  <ListTypeInfo extends BaseListTypeInfo>({
    ...
    defaultValue,
    ...config
  }: CustomTimestampFieldConfig<ListTypeInfo> = {}): FieldTypeFunc<ListTypeInfo> =>
  (meta) => {
    ...
    return fieldType({
      customConfig: {
        hasNowButton: config?.customConfig?.hasNowButton,
        hideTime: config?.customConfig?.hideTime,
      },
      kind: 'scalar',
      mode,
      scalar: 'DateTime',
      ...
      getAdminMeta(): CustomTimestampFieldMeta {
        return {
          customConfig: {
            hasNowButton: config?.customConfig?.hasNowButton || false,
            hideTime: config?.customConfig?.hideTime || false,
          },

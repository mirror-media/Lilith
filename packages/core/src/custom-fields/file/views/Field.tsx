...
function FileView({
  ...
  let correctUrl = ''
  let fileType = ''
  const wrongUrl: string = value?.data?.src
  if (wrongUrl) {
    ...
    correctUrl = urlArray.filter(function (el) { return el != '' }).join('/')
    correctUrl = correctUrl.replace('https:/', 'https://')
	//FIXME: the walkaround solution to be correctted 
  }
          ...
          {value.kind === 'from-server' && (
            <Stack padding="xxsmall" gap="xxsmall">
              <Stack across align="center" gap="small">
                <Text size="medium">
                  {fileType === 'image' && correctUrl ? (
                    <a href={correctUrl} target="_blank">
                      <img
                        src={correctUrl}

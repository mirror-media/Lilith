...
export const CustomRelationship =
  <ListTypeInfo extends BaseListTypeInfo>({
    ref,
    ...config
  }: CustomRelationshipFieldConfig<ListTypeInfo>): FieldTypeFunc<ListTypeInfo> =>
  (meta) => {
    ...
    const commonConfig = {
      ...
      getAdminMeta: (
        ...
        if (config.ui?.displayMode === 'cards') {
          ...
          const currentField = adminMetaRoot.listsByKey[
            meta.listKey
          ].fields.find((x) => x.path === meta.fieldKey)
          if (currentField) {
        ...
        return {
          customConfig: {
            isImage: config.customConfig?.isImage,
          },
    ...
    if (config.many) {
      return fieldType({
        ...
        customConfig: {
          isImage: config.customConfig?.isImage || false,
        },
      })({
        ...commonConfig,
        input: {
        ...
        extraOutputFields: {
          [`${meta.fieldKey}Count`]: graphql.field({
            ...
            args: {
              where: graphql.arg({
                type: graphql.nonNull(listTypes.where),
                defaultValue: {},
              }),
            },
            resolve({ value }, args) {
